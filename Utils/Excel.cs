...
namespace UniversitySchedule.Utils
{
    public class Excel
    {
        ...
        public static void ExportCoursesToExcelWithDialog(List<Class> Classes)
        {
            try
            {
                using (SaveFileDialog saveFileDialog = new SaveFileDialog())
                {
                    ...
                    if (saveFileDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        using (var workbook = new XLWorkbook())
                        {
                            ...
                            {
                                worksheet.Cell(row, 1).Value = ++index;
                                worksheet.Cell(row, 2).Value = classDetail.Department.Name;
                                worksheet.Cell(row, 3).Value = classDetail.Course.Name;
                                worksheet.Cell(row, 4).Value = (int)classDetail.MeetingTime.Day + 1;
                                worksheet.Cell(row, 5).Value = classDetail.MeetingTime.ToString();
                                worksheet.Cell(row, 6).Value = classDetail.Room.ToString();
                                worksheet.Cell(row, 7).Value = classDetail.Instructor.User.Information.Name;
                                worksheet.Cell(row, 8).Value = classDetail.Course.MaxStudent;
                                row++;
                            }
